...
public:
  ...
  bool factoryDefault() {
    ...
    waitResponse();
	sendAT(GF("E&W"));		// Factory + Reset + Echo Off + Write
	waitResponse();
    ...
    waitResponse();
	delay(5000);
	sendAT(GF("+CLTS=1"));	// Get local time from mobile network
	if (waitResponse(10000L) != 1) {
		return false;
	}
	sendAT(GF("+CENG=3"));// Enable engineering mode
	if (waitResponse(10000L) != 1) {
		return false;
	}
	sendAT(GF("+CTZU=1"));	// Enable local time zone
	if (waitResponse(10000L) != 1) {
		return false;
	}
